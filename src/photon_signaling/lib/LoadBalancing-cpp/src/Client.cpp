...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		//TODO: this is a temporary workaround for the "getanswer truncated answer error: 1477" issue with the traffic manager on Nintendo Switch
		const JString Client::M_NAMESERVER = L"nameserver-eu.cloudapp.net"; // default name server address
#else
		...
		/**
		   ...
		   not be able
		   to be matched with each other or to even see each other, i.e. incompatible versions of your game or public, closed-beta, QA, staging and dev clients. This parameter gets ignored by
		   %Photon Server.
		   ...
		   @sa Listener, ConnectionProtocol, RegionSelectionMode */
...
#	pragma warning(pop)
		...
		{
#ifdef _EG_USE_WSS_ON_NS_BUT_SPECIFIED_PROTOCOL_ELSEWHERE
//#error TODO: remove this block when the server starts to support sending customInitData over websockets
// currently websockets can't be used for connections to other server types than nameservers when mUseAuthOnce==true
#	ifdef _EG_XB1_PLATFORM
			if(M_CONNECTION_PROTOCOL == ConnectionProtocol::WSS)
#	else
			if(M_CONNECTION_PROTOCOL == ConnectionProtocol::WS || M_CONNECTION_PROTOCOL == ConnectionProtocol::WSS)
#	endif
			{
				EGLOG(DebugLevel::OFF, L"unsupported connection protocol %hhu", M_CONNECTION_PROTOCOL);
				DEBUG_ASSERT(false);
			}
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype& parameters, nByte eventCode, RaiseEventOptions options)
		   Sends in-game data to all other players in the game, who will receive it in their Listener::customEventAction() callback.
		   @details
		   ...
		   This function provides the option to raise events reliable or unreliable. While both result in ordered events, the ones that got sent with the latter option
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype pParameterArray, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize, nByte eventCode, RaiseEventOptions options)
		   @overload
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype pParameterArray, const short* pArrSizes, nByte eventCode, RaiseEventOptions options)
		   @overload
		   @param reliable true = operation will be sent reliably; false = no resend in case of packet loss - will be ignored, when not using UDP as protocol
		...
		/**
		   ...
		   @details
		   This function sends a request to the server to join the specified lobby. If it returns true, then Listener::joinLobbyReturn() gets called when the operation has successfully bee
		   finished.
		   @remarks
		...
		/**
		   ...
		   @param gameID The name to create a room with. Must be unique and not in use or the room can't be created. If null, then the server will assign a GUID as name.
		...
		/**
		   ...
		   @param gameID A unique identifier for the game room to join or create
		...
		/**
		   ...
		   can not be considered, as a lobby name only needs to be unique among lobbies of the same type.
		...
		/**
		   ...
		   default is 'false'.
		   @param sendAuthCookie Pass 'true' to set the sendAuthCookie web flag (please see https://doc.photonengine.com/en-us/realtime/current/reference/webhooks-v1-2 for further information).
		...
		/**
		   ...
		   @returns true, if the request could successfully be queued for sending to the server, false otherwise.
		   @sa
		   opRaiseEvent(), RaiseEventOptions::setInterestGroups(), https://doc.photonengine.com/en-us/realtime/current/reference/interestgroups */
		...
		/**
		   ...
		   to the server that you pass when calling connect() and the server does not respond with a final result (successful connect or failed connect attempt dues to an authentication error),
		...
		/**
		   ...
		   The list of available regions for Photon Public Cloud is available at https://doc.photonengine.com/en-us/realtime/current/reference/regions.
		...
		/**
		   ...
		   @returns true, if the request could successfully be queued for sending to the server, false otherwise.
		   @sa
		   Listener::webRpcReturn(), @link Datatypes Table of Datatypes\endlink, https://doc.photonengine.com/en-us/realtime/current/reference/webhooks
		...
		{
			if(address.indexOf('.') > -1 || address.indexOf(L"localhost") > -1) // IPv4 or URL or localhost
				return address+(address.indexOf(L':')<0?JString(L":")+getDefaultPort(serverType):JString());
			else // IPv6
			{
				int endIndex, idx, idx2 = address.substring(idx=address.indexOf(']')).indexOf(':');
				endIndex = idx2==-1?idx2:idx;
				return endIndex<0?JString(L"[")+address+JString(L"]:")+getDefaultPort(serverType):address;
			}
		}

		unsigned short Client::getDefaultPort(nByte serverType)
		{
			...
			return serverType==ServerType::NAME_SERVER?port+NAMESERVER_PORT_GAP:port;
		}
	}
}