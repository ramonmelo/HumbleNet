...
#	define _EG_MS_COMPILER true // Marmalade really even defines _MSC_VER, when using another compiler, so we have to additionally check, that no other compiler (of the used by Marmalade ones) is running
#endif
...
#if defined EG_PLATFORM_SUPPORTS_CPP11 && ((!defined _EG_LINUX_PLATFORM) || (__GNUC__ >= 5 || __GNUC__ == 4 && (__GNUC_MINOR__ >= 8)))
