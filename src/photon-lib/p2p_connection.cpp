...
namespace humblenet {
	ha_bool P2PSignalConnection::processMsg(const HumblePeer::Message* msg)
	{
		...
		switch (msgType) {
			...
			{
				LOG_INFO("P2PAnswer from peer %u\n", peerId);
			...
			{
				LOG_INFO("ICECandidate from peer %u\n", peerId);
			...
			{
				LOG_INFO("P2PReject from peer %u\n", peerId);
			...
			{
				LOG_INFO("HelloServer from peer %u\n", peerId);
...
// 				sendHelloClient(this, peerId, reconnectToken, iceServers);
			...
			{
				LOG_INFO("P2PRelayData from peer %u\n", peerId);
			...
			{
				LOG_INFO("AliasRegister from peer %u\n", peerId);
	...
	// 			}
			...
			{
				LOG_INFO("AliasUnregister from peer %u\n", peerId);
	...
	// 			}
			...
			{
				LOG_INFO("AliasLookup from peer %u\n", peerId);
